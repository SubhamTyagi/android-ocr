...
fun DownloadLanguageDataScreen(/* viewModel: DownloadLanguageViewModel= viewModel(),*/
    ...
    val items = keys.zip(names) { key, name ->
        Language(
            ...
            name,
            isDownloaded = true,
            isSelected = true/*selectedLanguages.value.contains(key)*/
        )
    }
    Column {
        Text(
        ...
        val list by remember { mutableStateOf(items) }
...
fun SearchBar(modifier: Modifier = Modifier) {
    ...
    TextField(
        ...
        modifier = modifier
            .fillMaxWidth()
            .heightIn(min = 48.dp), leadingIcon = {
            Icon(
...
fun LanguageCard(
    ...
    Card(
        ...
        Row(
            ...
            Image(
                painter = if (isDownloaded) painterResource(R.drawable.baseline_delete_24) else painterResource(
                    R.drawable.baseline_download_24
                ),
                contentScale = ContentScale.Crop,
                contentDescription =  if (isDownloaded) "Delete the data" else "Download the data",
                modifier = Modifier
                    .size(30.dp)
                    .clickable(true) {
                        if (isDownloaded) {
