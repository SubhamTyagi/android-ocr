...
fun DownloadLanguageDataScreen(
    ...
    val languageList: List<Language> by remember { mutableStateOf(downloadViewModel.languageList) }
    var downloadProgress = downloadViewModel.downloadProgress.collectAsState()
    val updatedLanguageList = languageList.map { language ->
        language.copy(isSelected = selectedLanguages.value.any { it.code == language.code })
    }
    ...
    DownloadLanguageDataScreenP(
        ...
        onSelected = { language, value ->
            if (value) {
                selectedLanguages.value.plus(language.code)
                if (!downloadViewModel.isLanguageDataExist(language)) {
                    downloadViewModel.downloadLanguage(language)
                }

            } else {
                selectedLanguages.value.minus(language.code)
            }
...
fun DownloadLanguageDataScreenP(
    ...
    val showDownloadProgressBar by remember { mutableStateOf(false) }
    Column {

        if (showDownloadProgressBar){
            ProgressBar(downloadProgress.value)
        }
...
fun PreviewDownloadLanguageDataScreen() {
    CharacherRecognizerTheme {
        ...
        val showDownloadProgressBar= remember { mutableFloatStateOf(15f) }

        DownloadLanguageDataScreenP(
            languageList = items,
            navController = rememberNavController(),
            downloadProgress = showDownloadProgressBar,
